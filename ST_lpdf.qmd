...
\end{align}
Here $\mathbf{I}_{T}$ denotes the identity matrix, $\mathbf{I}_{T}^{*}$ the diagonal matrix whose entries are $(0,1,\ldots,1,0)$, $\mathbf{D}_{S}$ the diagonal matrix whose entries are $(n_{1},n_{2},\ldots,n_{S})$ with $n_{s}$ the number of neighbors of region $s$, whereas $\mathbf{A}_{T}$ and $\mathbf{A}_{S}$ the adjacency matrices of the temporal AR and spatial CAR structure, respectively.    
